...
function object_array_contains($arr, $property, $item) {
  return count(
    array_filter(
      $arr,
      function ($arr_item) use ($item, $property) {
...
class NaiveBayesClassifier {
  ...
  public function classifyMultiple($sentence) {
    $keywordsArray = $this->tokenize($sentence);
    $categories = $this->decideMultiple($keywordsArray);
    return $categories;
  }
  ...
  private function decideMultiple($keywordsArray) {
    $categories = [];
    ...
    foreach ($counts as $count) {
      ...
      // echo $count['name'] . ': ' . $probability . '<br/>';
      $categories[$count['name']] = $probability;
    }
    return $categories;
  }
